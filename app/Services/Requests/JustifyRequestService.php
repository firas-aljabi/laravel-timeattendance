...
class JustifyRequestService
{
    ...
    public function reject_justify_request($id)
    {
        if (auth()->user()->type == UserTypes::ADMIN) {

            $justifyAfterReject = $this->justifyRequestRepository->getById($id);
            $justifyAfterReject->status = JustifyStatus::REJECTED;
            $justifyAfterReject->update();
            return ['success' => true, 'data' => $justifyAfterReject->load('user')];
        } else {
            return ['success' => false, 'message' => "Unauthorized"];
        }
    }
