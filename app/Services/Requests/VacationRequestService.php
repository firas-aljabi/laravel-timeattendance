...
class VacationRequestService implements VacationServiceInterface
{
    ...
    public function reject_vacation_request($id)
    {
        if (auth()->user()->type == UserTypes::ADMIN) {
            ...
            $vacationAfterReject = $this->vacationRequestRepository->getById($id);
            $vacationAfterReject->status = VacationRequestStatus::REJECTED;
            $vacationAfterReject->update();
            return ['success' => true, 'data' => $vacationAfterReject->load('user')];
        } else {
            return ['success' => false, 'message' => "Unauthorized"];
        }
    }
